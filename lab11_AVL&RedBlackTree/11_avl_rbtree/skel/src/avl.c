...
void avl_delete(Node **node, void *value, int data_size, int (*compar)(void*, void*))
{
	...
	if (compar((*node)->data, value) > 0) {
		...
		else if (height((*node)->right) <= 0 && height((*node)->left) < 0) {
			Node *aux = (*node)->right;
			free((*node)->data);
			free(*node);
			*node = aux;
		...
		// daca are 2 fii il inlocuim cu maximul dintre ele
		else {
			Node *max_node = max_element((*node)->left);
			memcpy((*node)->data, max_node->data, data_size);
			avl_delete(&(*node)->left, (*node)->data, data_size, compar);
		}
	}
